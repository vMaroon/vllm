...
def run_simulation(
    ...
    print(f"Priority: {priority_fraction*100:.0f}% of blocks at "
    ...
    for i in range(num_sessions):
        ...
        if free < blocks_per_session:
            print(f"  Session {i}: only {free} free blocks, "
        ...
        print(f"  Session {i}: allocated {len(blocks)} blocks "
    ...
    if eviction_target <= total_unprioritized:
        ...
        if evicted_prioritized == 0:
            print("\n  PASS: No prioritized blocks evicted "
                  "(all evictions from LRU)")
        else:
        ...
        if evicted_unprioritized == total_unprioritized:
            ...
            print(f"  FAIL: Only {evicted_unprioritized}/{total_unprioritized}"
    ...
    return evicted_prioritized == 0 or \
        evicted_unprioritized == len(unprioritized_block_ids)


def main():
    parser = argparse.ArgumentParser(
        description="Benchmark retention-based eviction")
    parser.add_argument("--num-gpu-blocks", type=int, default=1000,
    ...
    parser.add_argument("--blocks-per-session", type=int, default=40,
                        help="Blocks per session")
    parser.add_argument("--priority-fraction", type=float, default=0.3,
                        help="Fraction of blocks with priority (0-1)")
    parser.add_argument("--priority-value", type=int, default=80,
