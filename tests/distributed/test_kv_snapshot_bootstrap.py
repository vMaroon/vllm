...
from examples.others.kv_snapshot_poc.check_residency import apply as audit
...
def test_residency_auditor_accepts_current_and_legacy_wire(array_format):
    event = store([1])
    event.locality = "LOCAL"
    event.ownership = "disk"
    raw = ["BlockStored", *msgspec.structs.astuple(event)] if array_format else event
    live: Counter = Counter()
    audit(live, base64.b64encode(msgspec.msgpack.encode([0, [raw], None])))
    assert live == Counter({("GPU", None, "LOCAL", "disk", 1): 1})
