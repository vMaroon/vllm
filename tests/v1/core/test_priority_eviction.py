...
class TestPriorityEvictionQueue:
    ...
    def _make_block(self, block_id: int, priority: int,
                    freed_time: float = 0.0,
                    expiry: float | None = None) -> KVCacheBlock:
...
class TestBlockPoolPriorityEviction:
    ...
    def _make_pool(self, num_blocks: int = 10,
                   enable_caching: bool = True) -> "BlockPool":
    ...
    def test_free_unprioritized_goes_to_lru(self):
        ...
        assert pool.free_block_queue.num_free_blocks == \
            pool.num_gpu_blocks - 1 - 3  # -1 for null block

        pool.free_blocks(blocks)
        assert pool.priority_eviction_queue.num_blocks == 0
        assert pool.free_block_queue.num_free_blocks == \
            pool.num_gpu_blocks - 1  # all back in LRU
    ...
    def test_free_prioritized_goes_to_priority_queue(self):
        ...
        assert pool.free_block_queue.num_free_blocks == \
            pool.num_gpu_blocks - 1 - 1  # -1 null, -1 in priority queue
    ...
    def test_get_num_free_blocks_sums_both(self):
        ...
        expected = pool.free_block_queue.num_free_blocks + \
